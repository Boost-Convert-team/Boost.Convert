...
DEFAULT_CONVERSION_ERROR = (
    "Nao foi possivel concluir a conversao deste arquivo. "
    "Tente enviar outro arquivo ou escolha outra configuracao."
)

def get_user_friendly_conversion_error(error):
    if isinstance(error, ValueError):
        return clean_error_message(str(error))

    if isinstance(error, FileNotFoundError):
        return "Nao foi possivel acessar o arquivo enviado. Envie o arquivo novamente."

    if isinstance(error, UnicodeDecodeError):
        return "Nao foi possivel ler o texto deste arquivo. Verifique a codificacao e tente novamente."

    if isinstance(error, subprocess.CalledProcessError):
        return "Nao foi possivel processar a midia enviada. Verifique se o arquivo abre normalmente e tente novamente."

    if is_ffmpeg_error(error):
        return "Nao foi possivel processar o audio ou video enviado. Verifique o arquivo e tente novamente."
...
def is_safe_runtime_message(message):
    safe_messages = {
        "Arquivo final nao foi criado.",
        "Arquivo final vazio.",
        "Envie pelo menos dois PDFs para juntar.",
    ...
    safe_prefixes = (
        "Pagina fora do intervalo permitido:",
        "Intervalo invalido:",
    )
...
def has_path_like_text(message):
    if os.sep in message or "/" in message:
        return True
