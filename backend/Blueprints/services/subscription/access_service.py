...
def lock_usage_identity(usuario=None, session_id=None) -> None:
    identity_type, identity_value = get_usage_identity(
        usuario=usuario,
    ...
    db.session.execute(
        text("SELECT pg_advisory_xact_lock(hashtext(:usage_key))"),
        {"usage_key": f"usage:{identity_type}:{identity_value}"},
    )
...
def reset_usage_window(usage, now: datetime) -> None:
    usage.window_started_at = now
    usage.usage_date = now.date()
    usage.usage_count = 0


def ensure_active_window(usage, now: datetime) -> None:
...
def get_reset_at(usage) -> datetime | None:
    window_started_at = as_utc(usage.window_started_at)
...
def format_reset_in(reset_at: datetime | None, now: datetime | None = None) -> str | None:
    if reset_at is None:
        return None
...
def build_usage_status(usage, now: datetime | None = None) -> UsageStatus:
    now = now or utc_now()
    if usage is None:
        return UsageStatus(
            used=0,
    ...
    return UsageStatus(
        ...
        limit=FREE_DAILY_LIMIT,
        remaining=remaining,
        percent=min(round((used / FREE_DAILY_LIMIT) * 100), 100),
        reset_at=reset_at,
        reset_in=format_reset_in(reset_at, now),
        is_limited=True,
    )


def get_usage_status(usuario=None, session_id=None) -> UsageStatus:
    if is_pro_user(usuario):
        return UsageStatus(0, None, "Ilimitado", 100, None, None, False)
    if usuario is None and session_id is None:
        return build_usage_status(None)
    return build_usage_status(
        get_usage_record(usuario=usuario, session_id=session_id)
    )


def get_limit_message(usage, requested_amount: int, now: datetime | None = None) -> str:
    now = now or utc_now()
    ...
    if requested_amount == 1:
        return (
            "Voce atingiu o limite de 10 conversoes nesta janela de 24 horas. "
            f"Tente novamente em {reset_text}."
        )
    return (
        f"Voce tem {remaining} conversoes disponiveis nesta janela de 24 horas, "
        f"mas este envio precisa de {requested_amount}. Tente novamente em {reset_text}."
    )


def reserve_tool_usage(usuario=None, session_id=None, amount: int = 1):
    if amount < 1:
    ...
    usage = get_or_create_usage_record(
        usuario=usuario,
        session_id=session_id,
        now=now,
        for_update=True,
    ...
    ensure_active_window(usage, now)
    if usage.usage_count + amount > FREE_DAILY_LIMIT:
        raise UpgradeRequiredError(get_limit_message(usage, amount, now))
