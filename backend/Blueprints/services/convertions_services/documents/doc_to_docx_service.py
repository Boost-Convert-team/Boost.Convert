...
def convert_doc_docx(input_path, output_path): run_libreoffice_conversion(input_path, output_path, "docx")
...
def find_converted_file(temp_dir, input_path, output_extension):
    expected_path = os.path.join(temp_dir ,  f"{os.path.splitext(os.path.basename(input_path))[0]}.{output_extension}")
