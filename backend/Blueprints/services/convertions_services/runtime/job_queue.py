...
from Blueprints.services.convertions_services.validation.file_security import remove_file_quietly, remove_job_input_quietly
from Blueprints.services.privacy.audit import update_conversion_audit_status
...
def submit_conversion_job(app, job_id, convert_function, runtime_options=None):
    executor.submit(process_conversion_job, app, job_id, convert_function, runtime_options)

def process_conversion_job(app, job_id, convert_function, runtime_options=None):
    with app.app_context():
        ...
        update_conversion_audit_status(job, "processing")
        db.session.commit()
        try:
            options = runtime_options if runtime_options is not None else job.options or {}
            convert_with_options(convert_function, job.input_path, job.output_path, options)
            ...
            update_conversion_audit_status(job, "done")
            db.session.commit()

        except Exception as exc:
            app.logger.error("Erro ao processar job de conversao: %s", type(exc).__name__, exc_info=False)
            remove_job_input_quietly(job.input_path)
            remove_file_quietly(job.output_path)

            job.status = "failed"
            job.options = {}
            job.error_message = get_user_friendly_conversion_error(exc)
            job.finished_at = utc_now()
            update_conversion_audit_status(job, "failed")
            db.session.commit()
