...
def run_conversion_file_cleanup(app):
    ...
    try: cleanup_old_conversion_files(app)
    except Exception: app.logger.exception("Erro ao limpar arquivos antigos de conversao")
...
def cleanup_old_conversion_files(app):
    ...
    jobs = ConversionJob.query.filter(ConversionJob.status.in_(["done", "failed"])).filter(ConversionJob.finished_at.isnot(None)).filter(ConversionJob.finished_at < cutoff).all()
    conversions_root = os.path.abspath(os.path.join(app.instance_path, "conversions"))

    for job in jobs: remove_job_files(job, conversions_root)

def remove_job_files(job, conversions_root):
    job_dirs = []
    if job.input_path and os.path.isdir(job.input_path): job_dirs.append(job.input_path)
    if job.output_path: job_dirs.append(os.path.dirname(job.output_path))

    removed_dir = False

    for job_dir in job_dirs:
        if remove_directory_inside_root(job_dir, conversions_root): removed_dir = True
    if removed_dir: return
...
def remove_directory_inside_root(path, root):
    ...
    if not is_inside_root(directory, root) or not os.path.isdir(directory): return False
...
def remove_file_inside_root(path, root):
    ...
    if not is_inside_root(file_path, root) or not os.path.isfile(file_path): return
