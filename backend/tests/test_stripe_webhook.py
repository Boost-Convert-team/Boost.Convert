...
class StripeWebhookTests(unittest.TestCase):
    ...
    def test_invoice_paid_activates_and_renews(self):
        ...
        with (
            ...
            result = process_event(
                self.event("evt_paid", "invoice.paid", self.invoice(period_end))
            )
            subscription = Subscription.query.one()
            user = db.session.get(Usuario, self.user_id)
        ...
        with (
            ...
            patch(
                ...
                return_value=self.subscription(
                    status="past_due", period_end=period_end
                ),
            ),
        ):
            db.session.add(
            ...
            process_event(
                self.event(
                    "evt_failed", "invoice.payment_failed", self.invoice(period_end)
                )
            )
            subscription = Subscription.query.one()
        ...
        with self.app.app_context():
            ...
            process_event(
                self.event(
                    ...
                    self.subscription("active", period_end, cancel_at_period_end=True),
                )
            )
            self.assertEqual(db.session.get(Usuario, self.user_id).plano, "pro")
            self.assertTrue(Subscription.query.one().cancel_at_period_end)
            process_event(
                self.event(
                    "evt_deleted",
                    "customer.subscription.deleted",
                    self.subscription("canceled", period_end),
                )
            )
    ...
    def test_unknown_event_is_idempotently_ignored(self):
        with self.app.app_context():
            result = process_event(
                self.event("evt_unknown", "customer.created", {"id": "cus_user"})
            )
